...
<?
	...
	$total_sql = " select *, (select subject from tbl_bbs_category where tbl_bbs_category.tbc_idx=tbl_bbs_list.category) as category from tbl_bbs_list where code='$code' and bbs_idx='".$bbs_idx."'";
	$result = mysql_query($total_sql) or die (mysql_error());
	$row=mysql_fetch_array($result);
	$reply = $row[reply];
	for ($i=1;$i<=5;$i++) {
				...
				<div>
					<table class="ta_write01">
						...
						<tbody>
							...
							<tr>
								<th>등록일시</th>
								<td colspan="3"><?=str_replace("-",".",substr($row[r_date],0,10))?></td>
							</tr>
							<tr>
								<th>분류</th>
								<td>
