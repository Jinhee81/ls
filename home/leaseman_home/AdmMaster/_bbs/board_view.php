...
<?
	...
	//후기 게시판 휴대폰 번호 이메일 멤버 테이블에서가져오기
	$m_sql ="select mobile, user_email from tbl_customer where user_id ='".$row['user_id']."'";
	$m_result =mysql_query($m_sql);
	$m_row = mysql_fetch_array($m_result);
	if($code =="h_after"){
		$email = $m_row['user_email'];
		$user_hp = $m_row['mobile'];
	}
?>
