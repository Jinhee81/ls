  ...
  <tr>
    ...
    <td>
        <select class="form-control" name="screenName" disabled>
            <option value="세입자화면"<?php if($allRows[$i]['screen']==='세입자화면'){echo "selected";}?>>세입자화면</option>
            <option value="방계약화면"<?php if($allRows[$i]['screen']==='방계약화면'){echo "selected";}?>>방계약화면</option>
            <option value="입금예정화면"<?php if($allRows[$i]['screen']==='입금예정화면'){echo "selected";}?>>입금예정화면</option>
            <option value="입금완료화면"<?php if($allRows[$i]['screen']==='입금완료화면'){echo "selected";}?>>입금완료화면</option>
        </select>
    </td>
    <td><?=$allRows[$i]['title']?>
...
<script>
var newTr2 = "<td colspan='5' style='padding-left:20px;'><p class='grey'>*상용구항목<br>세입자화면 : {세입자},{이메일}<br>방계약화면 : {세입자},{이메일},{계약일},{종료일}<br>입금예정화면 : {세입자},{이메일},{예정일},{합계},{시작일},{종료일},{개월수},{연체일수},{연체이자}<br>입금완료화면 : {세입자},{이메일},{입금일},{발행일},{공급가액},{세액},{합계}<br></p></td>";


$('#addstring').on('click', function(){
    var td1 = "<td></td>";
    var td2 = "<td><select name='screen' class='form-control'><option value='세입자화면'>세입자화면</option><option value='방계약화면'>방계약화면</option><option value='입금예정화면'>입금예정화면</option><option value='입금완료화면'>입금완료화면</option></select></td>";
