<!-- 이 파일은 저장버튼후 로딩되기위해 get을 쓰기위해 만들어진 파일, 일단 냅둔다. get방식이 필요해서 ㅠㅠ -->
<?php
session_start();
...
<?php print_r($_GET); var_dump($_GET['year']); ?>
        ...
        <div class="form-group row justify-content-md-center">
            <div class="col-sm-2 pl-0 pr-1">
              <select class="form-control form-control-sm selectCall" name="building">
                <option value="<?=$_GET['buildingIdx']?>"><?=$_GET['building']?></option>
              </select>
            </div>
            <div class="col-sm-2 pl-0 pr-1">
              <select class="form-control form-control-sm selectCall" name="year">
                <option value="<?=$_GET['year']?>"><?=$_GET['year'].'년'?></option>
              </select>
            </div>
            <div class="col-sm-2 pl-0 pr-1">
              <select class="form-control form-control-sm selectCall" name="month">
                <option value="<?=$_GET['month']?>"><?=$_GET['month'].'월'?></option>
              </select>
            </div>
...
$('#btnSave').on('click', function(){
    ...
    for (var i = 0; i < flexrow; i++) {
      ...
      var costlist2Title = $('#flexcostTable tbody').find("tr:eq("+i+")").find("td:eq(1)").children('input').val(); //내역
