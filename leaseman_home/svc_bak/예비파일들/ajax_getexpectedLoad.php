...
$(":checkbox:first", table).click(function(){
  ...
  if($(":checkbox:first", table).is(":checked")){
    for (var i = 1; i <= allCnt; i++) {
      ...
      smsReadyArrayEle.push(colOrder, colid, colgroup, colroom, colcustomerName, colcustomerContact)
...
if($(this).is(":checked")){
  ...
  var colcustomerName = currow.find("td:eq(4)").children('input:eq(0)').val();
  var colcustomerContact = currow.find("td:eq(4)").children('input:eq(1)').val();
  var dropReady = smsReadyArrayEle.push(colOrder, colid, colgroup, colroom, colcustomerName, colcustomerContact);
  var index = smsReadyArray.indexOf(dropReady);
  smsReadyArray.splice(index, 1);
}
// console.log(smsReadyArray);
// console.log(typeof(smsReadyArray[3]));
})
