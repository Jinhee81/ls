...
if(count($allRows)===0){
  ...
  $j = count($allRows); //내림차순을 위해 만든 변수
  for ($i=0; $i < count($allRows); $i++) {
    ?>
    <tr>
      ...
      <td class='text-center'><a href="m_c_edit.php?id=<?=$allRows[$i]['id']?>" data-toggle="tooltip" data-placement="top" title="<?=$allRows[$i]['cName']?>">
        <?=mb_substr($allRows[$i]['cName'],0,20,"utf-8")?></a>
        <?php
      ...
      <td class="mobile">
        <?php
            if($allRows[$i]['div1']==='세입자'){
              echo "<a class='btn btn-info btn-sm' href='/svc/service/contract/contract_add1.php?id=".$allRows[$i]['id']."' role='button'>방계약</a>";
            }

            if($allRows[$i]['div1']==='기타'){
              echo "<a class='btn btn-info btn-sm' href='/svc/service/contractetc/contractetc_add1.php?id=".$allRows[$i]['id']."' role='button'>기타계약</a>";
            }
         ?>
      </td>
    </tr>
