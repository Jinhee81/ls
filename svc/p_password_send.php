...
if($row1[0] === 0) {
  ...
  history.back();
    ...
    if($row2['cellphone'] != $cellphone) {
        ...
        echo "<script>
        let newpassword = prompt('새로운 비밀번호를 입력해주세요.');
        console.log(newpassword);
        </script>";
        $newpassword = "<script>document.write(newpassword);</script>";

        print_r($newpassword);

        $hash = md5($newpassword);

        print_r($hash);echo 333;

        $sql3 = "update user set password = '{$hash}' where id={$row2['id']}";
