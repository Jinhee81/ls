...
<div class="container" style="max-width:500px;">
  <form method="post" action ="signup_proccess.php" class="form-signin" id="signup_form">
    ...
    <div class="form-group">
      <label>연락처</label><br>
      <input type="text" name="cellphone" class="form-control" id="phone" placeholder="휴대폰번호" required>
      <button type="button" onclick="phone_check();" name="cellphone_auth" id="phoneBtn" class="btn btn-sm btn-primary btn-block">휴대폰번호인증</button>
      <p id="check"></p>
...
function phone_check(){
    if($('#phone').val().length <1){
        ...
        SetTime = 180;
        $('#check').html(
            '<input type="text" name="sms" id="sms" class="form-control" placeholder="인증번호" required>' +
            '<button type="button" onclick="sms_check('+rand+');" name="sms_auth" id="smsBtn" class="btn btn-sm btn-primary btn-block">인증번호 확인</button>'
        );
        $.ajax({
        url: 'smscheck.php',
        method: 'post',
