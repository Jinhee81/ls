<!-- 지울예정임, 이유는 청구스케줄을 자바스크립트로 생성하는거에서 데이타베이스에서 불러오는걸로 바꿨기 때문 -->
<?php
session_start();
    ...
    <div class="p-3 mb-2 text-dark border border-info rounded">
      ...
      <div class="container form-row">
          <div class="form-group col-md-2">
                <button type="button" name="button" class="btn btn-outline-info btn-sm mobile">추가</button>
...
?>
<tr>
  <td><input type='checkbox' class='checkSelect' name='chk[]' value='<?=$row['idcontractSchedule']?>'></td>
  <td><?=$row2['ordered']?></td>
  <td><?=$row2['mStartDate']?></td>
  <td><?=$row2['mEndDate']?></td>
  <td>
    <input type='text' size='10' class='form-control form-control-sm text-right amountNumber' name='mAmount' value='<?=$row2['mMamount']?>'>
    <input type='text' size='10' class='form-control form-control-sm text-right amountNumber' name='mAmount' value='<?=$row2['mVmAmount']?>'>
  </td>
  <td>
    <input type='text' size='10' class='form-control form-control-sm text-right amountNumber' name='mAmount' value='<?=$row2['mTmAmount']?>'>
  </td>
  <td>
    <input type='text' size='10' class='form-control form-control-sm text-center dateType' name='expecteDay' value='<?=$row2['mExpectedDate']?>'>
  </td>
  <td></td>
  <td></td>
  <td></td>
...
<script>
    $(document).ready(function(){
      ...
      for (var i = 1; i <= Number(period); i++) {
        ...
        if(payOrder==='선불'){
          var expecteDay = "<input type='text' size='10' class='form-control form-control-sm text-center dateType' name='expecteDay' value='"+dateFormat(startDateInline)+"'>";
        } else if (payOrder==='후불') {
          var expecteDay = "<input type='text' size='10' class='form-control form-control-sm text-center dateType' name='expecteDay' value='"+dateFormat(endDateInline)+"'>";
        }

        scheduleTable += "<tr><td>"+checkbox+"</td><td>"+i+"</td><td>"+dateFormat(startDateInline)+"</td><td>"+dateFormat(endDateInline)+"</td>"+"<td>"+"<input type='text' size='10' class='form-control form-control-sm text-right amountNumber' name='mAmount"+i+"' value='<?=$row['mAmount']?>' numberOnly>"+"<input type='text' size='10' class='form-control form-control-sm text-right amountNumber' name='mvAmount"+i+"' value='<?=$row['mvAmount']?>' numberOnly>"+"</td>"+"<td>"+"<input type='text' size='10' class='form-control form-control-sm text-right amountNumber' name='mtAmount"+i+"' value='<?=$row['mtAmount']?>' numberOnly>"+"</td>"+"<td>"+expecteDay+"</td>"+"<td></td>"+"<td></td>"+"<td></td>"+"<td></td>"+"<td></td>"+"<td></td>"+"<td></td>"+"<td></td>"+"</tr>";
    ...
    $('#button1').click(function(){ //청구설정버튼 클릭시
      ...
      if(expectedDayArray.length === 0){
        ...
        goCategoryPage(aa, bb, cc, paySchedule);
