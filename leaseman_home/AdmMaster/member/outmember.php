...
<?
	...
	if ($sdate != "") {
		 $strSql .= " and e_date >= '".$sdate."'  ";
	}

	if ($edate != "") {
		 $strSql .= " and e_date <= '".$edate." 23:59:59'  ";
	}
	
	$total_sql = " 
			select c.* , ( select g_levels from tbl_payment where status = 'Y' and goods_type='g_level' and c_idx = c.c_idx ) as now_level
			from tbl_customer c where status = 1 
		".$strSql;
				...
				<form action="" method="get" >
					<fieldset>
						...
						<select name="cates" id="cates" class="mar_r10">
							<option value="c.user_code">회원번호</option>
							<option value="c.user_name">회원명</option>
							<option value="c.reco_id">추천인아이디</option>
						</select>
