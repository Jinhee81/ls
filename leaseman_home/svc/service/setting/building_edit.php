...
<section class="container">
  <div class="jumbotron">
    ...
    <p class="lead">
      ...
      (3) '형태' 변경을 원하는 경우 상단 <a href="/svc/user/myinfo.php"><i class="fas fa-user"></i>&nbsp;나의정보</a>에서 임대유형을 수정하세요.<br>
...
<section class="container" style="max-width:500px;">
  <form action="p_building_edit.php" method="post">
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-9">
        <select class="form-control" name="popbill" id="popbill">
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-9">
        <input type="text" class="form-control" id="cnum" name="companynumber" value="<?=$row['companynumber']?>">
      </div>
    </div>
    <div class="">
    ...
    <div class="mt-7">
      ...
      <button type="button" onclick="popbillYn()" class="btn btn-primary">확인</button>
    </div>
  </form>
</section>
...
function popbillYn(){
    ...
    success: function(data){
        if(data=='미가입'){
            $("#popbill").val("popbillno").prop("selected", true);
            $('#popbill').attr('disabled', 'true');
        }else if(data=='가입'){
            $("#popbill").val("popbillyes").prop("selected", true);
            $('#popbill').attr('disabled', 'true');
        }
