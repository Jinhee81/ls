...
function ipagelisting2($cur_page, $total_page, $n, $url) {

	$retValue = "<div class='paging'><ul>";
	if ($cur_page > 1) {
		$retValue .= "<li><a href='" . $url . "1' title='Go to next page'><img src='../img/btn/prev_left.png'></a></li>";
		$retValue .= "<li class='left_paging'><a href='" . $url . ($cur_page-1) . "' title='Go to first page'><img src='../img/btn/prev.png'></a></li>";
	} else {
		$retValue .= "<li><a href='javascript:;' title='Go to next page'><img src='../img/btn/prev_left.png'></a></li>";
		$retValue .= "<li class='left_paging'><a href='javascript:;' title='Go to first page'><img src='../img/btn/prev.png'></a></li>";
	}
	$retValue .= "";
	$start_page = ( ( (int)( ($cur_page - 1 ) / 10 ) ) * 10 ) + 1;
	$end_page = $start_page + 9;
	...
	if ($total_page == 0)
	{
		$retValue .= "<li><a href='javascript:;' title='Go to 0 page' class='active'>1</a></li>";
	} elseif ($total_page >= 1)
	{
		for ($k=$start_page;$k<=$end_page;$k++)
		{
			if ($cur_page != $k)
			{
				...
				$retValue .= "<li><a href='javascript:;' title='Go to ".$k." page' class='active'>".$k."</a></li>";
			}
		}
	}
	$retValue .= "";
	if ($cur_page < $total_page) {
		$retValue .= "<li class='right_paging'><a href='".$url.($cur_page+1) . "' title='Go to next page'><img src='../img/btn/next.png'></a></li>";
		$retValue .= "<li><a href='".$url.$total_page."' title='Go to last page'><img src='../img/btn/next_right.png'></a></li>";
	} else {
		$retValue .= "<li class='right_paging'><a href='javascript:;' title='Go to next page'><img src='../img/btn/next.png'></a></li>";
		$retValue .= "<li><a href='javascript:;' title='Go to last page'><img src='../img/btn/next_right.png'></a></li>";
	}
	$retValue .= "</ul></div>";
	return $retValue;
}
...
//function fn_chg_sms($in_msg, $text1, $text2, $text3, $text4, $text5, $text6, $text7, $text8, $text9, $text10 ){
function fn_chg_sms($in_msg, $text1, $text2, $text3, $text4, $text5, $text6, $text7, $text8, $text9 ){
	/*
	$text1 : {고객명}
	$text2 : {청구번호-예정금액}
	$text3 : {청구번호-예정일}
	...
	*/
	...
	/*
	if($text2){
		$in_msg = str_replace("{청구번호-예정금액}",$text2,$in_msg);
	}

	if($text3){
		$in_msg = str_replace("{청구번호-예정일}",$text3,$in_msg);
	}

	if($text4){
		$in_msg = str_replace("{청구번호-연체일수}",$text4,$in_msg);
	}

	if($text5){
		$in_msg = str_replace("{청구번호-연체이자}",$text5,$in_msg);
	}

	if($text6){
		$in_msg = str_replace("{청구번호-입금예정일}",$text6,$in_msg);
	}

	if($text7){
		$in_msg = str_replace("{청구번호-개월}",$text7,$in_msg);
	}

	if($text8){
		$in_msg = str_replace("{예정금액}",$text8,$in_msg);
	}

	if($text9){
		$in_msg = str_replace("{종료일}",$text9,$in_msg);
	}

	if($text10){
		$in_msg = str_replace("{청구번호-종료일}",$text10,$in_msg);
	}
	*/

	if($text2){
		$in_msg = str_replace("{예정금액}",$text2,$in_msg);
	}

	if($text3){
		$in_msg = str_replace("{예정일}",$text3,$in_msg);
	}

	if($text4){
		$in_msg = str_replace("{연체일수}",$text4,$in_msg);
	}

	if($text5){
		$in_msg = str_replace("{연체이자}",$text5,$in_msg);
	}

	if($text6){
		$in_msg = str_replace("{입금예정일}",$text6,$in_msg);
	}

	if($text7){
		$in_msg = str_replace("{개월}",$text7,$in_msg);
	}
...
// SMTP 서버에 E-mail 발송을 요청하는 함수
		...
		*/
		...
		function send_mail($from_name, $from_email, $to_name, $to_email, $subject, $message, $ext_header="", $file_arr="")
		{
			// 추가 설정
			if($ext_header != ""){
				$arr = explode("\n", $ext_header);
