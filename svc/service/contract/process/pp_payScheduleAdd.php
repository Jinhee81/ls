...
$a = json_decode($_POST['contractScheduleArray']);
// print_r($a);
//
for ($i=0; $i < count($a); $i++) {
  $sql3 = "
...
for ($i=0; $i < count($payExecutiveRow); $i++) {
  if($payExecutiveRow[$i][6]===0){
    ...
    echo json_encode('logical');//0원은 청구불가합니다.
    exit();
  }
}
