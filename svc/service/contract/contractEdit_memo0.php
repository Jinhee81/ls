<div class="p-3 mb-2 text-dark border border-info rounded">
  ...
  <table class="table table-sm table-bordered table-hover text-center mt-3">
    ...
    <tr>
        ...
        <td class="">
            <input class="form-control form-control-sm text-center" name="memoContent" value="<?=$memoRows[$i]['memoCreator']?>"></td>
        </td>
        <td class=""><textarea name="memoContent" class="form-control form-control-sm" rows="1" cols="80"><?=$memoRows[$i]['memoContent']?></textarea>
        <td class="mobile">
          <small class=""><?=$memoRows[$i]['created']?></small>
        </td>
        <td class="mobile">
          <small class="">
            <?php
             if(!$memoRows[$i]['updated']){
               echo "-";
             } else {
     ...
     <!-- <td><label class="grey"><//?=$memoRows[$i]['updated']?></label></td> -->
        <td id="append" class="mobile">
          ...
          <label class="small" name="memoEdit"><u>수정</u></label>&nbsp;
          <label class="small" name="memoDelete"><u>삭제</u></label>
        </td>
    </tr>
<?php }
} ?>
