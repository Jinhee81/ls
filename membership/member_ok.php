...
////////////////////////////////////////////////////////////
	...
	$email		= updateSQ($_POST['user_email']);
	$user_pw	= updateSQ($_POST['user_pw']);
	$user_name	= updateSQ($_POST['user_name']);
	$reco_id	= updateSQ($_POST['reco_id']);
	$year	    = updateSQ($_POST['year']);
	$month	    = updateSQ($_POST['month']);
	$day	    = updateSQ($_POST['day']);
	...
	$sign_up_path	= updateSQ($_POST['sign_up_path']);


//email test 
 // 회원님께 메일 발송
	$subject = $user_name.'님 회원가입을 축하드립니다.';
	ob_start();
	include_once ('./register_form_mail.php');
	$content = ob_get_contents();
	ob_end_clean();
	$admin_title ="리스맨";
	
	$from_email ="114@leaseman.co.kr";
	mailer($admin_title, $from_email, $email, $subject, $content, 1);
	...
	$sql = $sql .			   " ,pop_id "				        ;
	$sql = $sql .			   " ,com_num "				        ;
	$sql = $sql .			   " ,sign_up_path "				;
	$sql = $sql .			   " ,l_sdate "				        ;
	$sql = $sql .			   " ,l_edate) "					;
	$sql = $sql .	   "VALUES "						        ;
	$sql = $sql .			   " ('[%user_code%]' "			    ;
	$sql = $sql .			   " ,'[%user_id%]' "				;
	$sql = $sql .			   " ,old_password('[%user_pw%]') "		;
	$sql = $sql .			   " ,'[%com_type%]' "				;
	$sql = $sql .			   " ,'[%birthday%]' "				;
	$sql = $sql .			   " ,'[%user_name%]' "				;
	$sql = $sql .			   " ,'[%tel%]' "					;
	...
	$sql = str_replace("[%sign_up_path%]",			$sign_up_path,			$sql);
...
alert_msg("등록되었습니다.","./membership3.php");
?>