...
var smsReadyArrayEle = [];
    ...
    if($(this).is(":checked")){
      ...
      var colcustomerName = currow.find("td:eq(7)").children('input:eq(4)').val();
      var colcustomerCompany = currow.find("td:eq(7)").children('input:eq(6)').val();
      ...
      var colstartDate = currow.find("td:eq(1)").children('input[name=pStartDate]').val();
      var colendDate = currow.find("td:eq(1)").children('input[name=pEndDate]').val();
      var colmonthcount = currow.find("td:eq(1)").children('input[name=monthCount]').val();
      smsReadyArrayEle.push({'순번':colOrder}, {'청구번호':colid}, {'그룹':colgroup}, {'방번호':colroom}, {'받는사람':colcustomerName}, {'연락처':colcustomerContact}, {'이메일':colcustomerEmail}, {'납부일':colexecutiveDate}, {'증빙일':coltaxDate}, {'공급가액':colamount1}, {'세액':colamount2}, {'합계':colamount3}, {'받는사람id':colcustomerId},{'예정일':''}, {'시작일':colstartDate}, {'종료일':colendDate}, {'개월수':colmonthcount}, {'연체일수':''}, {'연체이자':''}, {'사업자명':colcustomerCompany});
