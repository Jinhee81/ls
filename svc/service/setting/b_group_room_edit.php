...
<section class="container">
  <div class="row justify-content-end pr-3 pb-2">
    <button type="button" class="btn btn-primary btn-sm mr-1" name="editbtn_group">그룹수정</button>
    <button type="button" class="btn btn-danger btn-sm" name="deletebtn_group">그룹삭제</button>
  </div>
  <table class="table table-bordered text-center" name="group">
    ...
    <tr>
      <td scope="col col-md-8"><input class="form-control text-center" type="text" name="building_name" value="<?=$row['bName'].'('.$row['bid'].','.$filtered_id.')'?>" disabled></td>
      <td scope="col col-md-8"><input class="form-control text-center" type="text" name="gName" required="" value="<?=$row['gName']?>"></td>
      <td scope="col col-md-8"><input class="form-control text-center" type="text" name="roomCount" disabled value="<?=$row['count']?>"></td>
      <td>
        <input class="form-control text-center" type="text" name="contractCount" disabled value="<?=$row2[0]?>">
      </td>
      <td><?=$row['created']?></td>
      <td><?=$row['updated']?></td>
      <!-- <td>
  ...
  <div class="row justify-content-end pr-3 pb-2">
    <button type="button" class="btn btn-outline-primary btn-sm" name="editbtn_room">저장</button>
      ...
      <tbody>
        ...
        while($row7 = mysqli_fetch_array($result7)){
          ...
          <tr>
            ...
            <td name="roomName">
              <input type="text" class="form-control form-control-sm text-center" value="<?=$row7['rName']?>" name="roomName">
            </td>
            ...
            </td><!--middle end contract-->
            <td name="ac">
              <?=$row7['total']?>
            </td><!--all contract-->
            <td>
              <span class="badge badge-warning" name="aboveInsert">위 삽입</span>
              <span class="badge badge-warning" name="belowInsert">아래 삽입</span>
              <button type="submit" class="btn btn-default" name="roomEditEach">
                <i class='far fa-edit'></i>
              </button>
              <button type="submit" class="btn btn-default pl-0" name="roomDeleteEach">
...
<script>
var aa = '<tr><td></td><td></td><td name="roomName"><input class="form-control form-control-sm text-center" name="roomName"></td><td></td><td></td><td></td><td></td><td></td><td></td><td class="pt-2"><span class="badge badge-danger" name="insertCansel">취소</span></td></tr>';

$('span[name=aboveInsert]').on('click', function(){
  var currow = $(this).closest('tr');
  currow.before(aa);

  $('span[name=insertCansel]').on('click', function(){
    var currow = $(this).closest('tr');
    currow.remove();
  })
})

$('span[name=belowInsert]').on('click', function(){
  var currow = $(this).closest('tr');
  currow.after(aa);
...
$('button[name=editbtn_room]').on('click', function(){
  ...
  for (var i = 0; i < allCnt; i++) {
    var roomArrayEle = [];
    var roomName = table.find('tr:eq('+i+')').find('td[name=roomName]').children('input[name="roomName"]').val();
