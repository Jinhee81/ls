...
$("#allselect").click(function(){
  ...
  if($("#allselect").is(":checked")){
    for (var i = 1; i <= allCnt; i++) {
      var colmonth = table.find("tr:eq("+i+")").find("td:eq(10)").children('a').text();
      var coldeposit = table.find("tr:eq("+i+")").find("td:eq(11)").children().text();
...
$(document).on('click', '.tbodycheckbox', function(){
    ...
    if($(this).is(":checked")){
      ...
      var colmonth = currow.find("td:eq(10)").children('a').text();
      var coldeposit = currow.find("td:eq(11)").children().text();
      colmonth = colmonth.replace(/,/gi,'');
      colmonth = Number(colmonth);
      coldeposit = coldeposit.replace(/,/gi,'');
      coldeposit = Number(coldeposit);
