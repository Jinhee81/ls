...
<div class="row justify-content-center">
  ...
  <?php include "contractEdit_button0.php";?>
  ...
  <nav class="container">
    <ul class="nav nav-tabs">
      <li class="nav-items">
        <a id="navSchedule" class="nav-link <?php if($_GET['page']==='schedule'){echo "active";} ?>" href="contractEdit.php?page=schedule&id=<?=$filtered_id?>">임대료목록(<?=$row['count2']?>개월)</a>
      </li>
      <li class="nav-items">
...
$(document).on('click', '.modalpay', function(){ //청구번호클릭하는거(모달클릭)
  ...
  var expectedAmount = $(this).siblings('input[name=ptAmount]').val();
  var expectedDate = $(this).siblings('span[name=pExpectedDate]').text();
  var executiveDiv = $(this).siblings('span[name=payKind]').text();//입금구분
  var executiveDate = $(this).siblings('span[name=executiveDate]').text();
  var executiveAmount = $(this).siblings('input[name=getAmount]').val();
  var payDiv = $(this).siblings('span[name=payDiv]').text();
  var taxMun = $(this).siblings('span[name=taxMun]').text();
  // alert(taxMun);

  // console.log(payNumber, expectedAmount);
...
$('#buttonDirect').on('click', function(){
  ...
  for (var i = 0; i < expectedDayArray.length; i++) {
    ...
    payScheduleEle.push(table.find("tr[name=contractRow]:eq("+expectedDayArray[i][0]+")").find('td[name=checkbox]').children('input[name=csId]').val()); //계약번호
    payScheduleEle.push(table.find("tr[name=contractRow]:eq("+expectedDayArray[i][0]+")").find('td[name=order]').children('span[name=ordered]').text()); //순번
    payScheduleEle.push(table.find("tr[name=contractRow]:eq("+expectedDayArray[i][0]+")").find('td[name=date]').find('input[mStartDate]').val()); //시작일
    payScheduleEle.push(table.find("tr[name=contractRow]:eq("+expectedDayArray[i][0]+")").find('td[name=date]').find('input[name=mEndDate]').val()); //종료일
    payScheduleEle.push(table.find("tr[name=contractRow]:eq("+expectedDayArray[i][0]+")").find('td[name=detail]').find('input[name=mAmount]').val()); //공급가액
    payScheduleEle.push(table.find("tr[name=contractRow]:eq("+expectedDayArray[i][0]+")").find('td[name=detail]').find('input[name=mvAmount]').val()); //세액
    payScheduleEle.push(table.find("tr[name=contractRow]:eq("+expectedDayArray[i][0]+")").find('td[name=detail]').find('input[name=mtAmount]').val()); //합계금액
    payScheduleEle.push(table.find("tr[name=contractRow]:eq("+expectedDayArray[i][0]+")").find('td[name=detail]').find('input[name=mExpectedDate]').val()); //예정일
