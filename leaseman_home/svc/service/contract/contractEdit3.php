...
$companynumber = $row['cNumber1'].'-'.$row['cNumber2'].'-'.$row['cNumber3'];
...
if($row['div2']==='개인사업자'){
  $cName = $row['name'].'('.$row['companyname'].','.$companynumber.')';
} else if($row['div2']==='법인사업자'){
  $cName = $cDiv3.$row['companyname'].','.$companynumber.'('.$row['name'].')';
} else if($row['div2']==='개인'){
  $cName = $row['name'];
}
...
// 500 에러
$edited_period = array($row_period[0], $row_startDate[0], $row_endDate[0]);
...
<section class="container">
  ...
  <div class="jumbotron text-center">
        <h1 class="display-4">>임대계약 상세 화면입니다!
          <small>
        <?php
        if($status==="진행"){
    ...
    <a class="btn btn-secondary" href="/svc/service/account/deposit.php" role="button">보증금리스트 화면으로</a>
       ...
       if($row3['payId'] && $row3['payIdOrder']==='0'){
        if($row3['executiveDate']) {
          ...
          if($row3['pExpectedDate'] >= $currentDate){
            ...
            echo "<p class='text-center font-weight-light pink'>(".number_format($row3['ptAmount']).")</p>";
          }
        }
    }
    ?>
...
<tr>
   ...
   <td>
     <button type="submit" name="memoEdit" class="btn btn-default grey" id="edit">
       <i class='far fa-edit'></i>
     </button>
     <button type="submit" name="memoDelete" class="btn btn-default grey" id="del">
       <i class='far fa-trash-alt'></i>
     </button>
   </td>
</tr>
...
<script src="https://www.leaseman.co.kr/admin/js/jquery-ui.min.js"></script>
<script src="https://www.leaseman.co.kr/admin/js/datepicker-ko.js"></script>
<script>
